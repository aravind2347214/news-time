...
export const NewsHeader: React.FC<NewsHeaderProps> = ({ title, origin }) => {
  ...
  return (
    <header
      ...
      className={` flex flex-col  xs:flex-row gap-5 items-center justify-center sm:justify-between p-5 bg-[#171717]`}
    >
      <div className={`flex  flex-row items-center w-full  ${origin==="search"?"justify-between":"justify-center"} `}>
        {origin === "search" && (
          <Button
            label="back button"
        ...
        {origin === "search" && <div className="w-5 h-5"></div>}
      </div>
